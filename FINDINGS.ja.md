...
東京都大学サッカー連盟が公開している試合記録から得た、公式サイトには掲載されていない5つの分析結果です。試合結果は2021年から、選手単位の記録は2022年からを対象にしています。

連盟が公開しているのは日程・順位表・得点ランキングです。元の記録には、選手ごとのシュート数、先発メンバー、時刻付きの交代、反則種別付きのカードも含まれますが、横断的な集計は公開されていません。以下では、連盟が公開している情報だけを使い、複数シーズンを同じ定義で集計しています。
...
| | 結果 | 要点 |
|---|---|---|
| [2](#2-学年差で一貫しているのは4年生の得点率だけ) | 学年 | 2026年の3年生に見えた傾向が、5シーズン並べると**消えた**（2025年の3年生が最高値だった）。否定的な結果としてそのまま載せています。 |
...
| [4](#4-チームごとの特徴を6指標で比較する) | チームの特徴 | 1シーズンの出場時間をどれだけ11人に集中させるか、どの相手から点を取るか。順位表では分からない差が出ます。 |
...
昇格した32件のうち29件で、翌シーズンの1試合平均勝点が低下しています。例外の3件はいずれも2022年の再編時で、3部と4部が統合された年に生じた昇格でした。
...
**この節に以前掲載していた非対称性の結論は撤回します。** 「昇格後の1試合平均勝点は約1.0下がる一方、降格後は約0.5しか上がらず、降格したチームの10件に3件はさらに悪化する」という主張でした。どちらも誤りです。チャレンジリーグは名称に番号がなく、2022〜2024年は3番目、2025年は4番目の階層でした。固定の対応表を使ったため、2022年に横移動または昇格していた13クラブを「降格」に分類していました。訂正後、降格したクラブの1試合平均勝点は+1.24となり、さらに悪化したのは17件中1件です。

## 2. 学年差で一貫しているのは4年生の得点率だけ
...
![チームの特徴](docs/figures/fig-fingerprints.png)

チームごとに6指標を計算し、そのシーズン内で0〜100に換算しています。指標はシュート数、決定率、守備、ターンオーバー（主力11人以外が占める出場時間の割合）、下級生比率（1・2年生が占める出場時間の割合）、後半の比重（後半に打ったシュートの割合）です。頂点の番号1〜6はこの順で、上から時計回りに並んでいます。レーダーチャートを並べることで、順位表だけでは分からないチームごとの特徴を比較できます。

各図の点線はリーグ平均です。どの指標もシーズン内で相対化しているため、この平均は必ずしも50にはなりません。多くのチームが上位に集まる指標では高く、少数のチームだけが低い指標では低くなります。
