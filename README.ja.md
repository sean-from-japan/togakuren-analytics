...
| **勝敗予測** | 設定は2022〜24年だけで決め、以降は変更していません。2025〜26年の525試合では対数損失が **1.0200 → 0.8192** となり、Elo の 0.8753を上回りました。正解率は44.6% → 65.7%。**最も寄与したのは時間減衰**で、外すと対数損失が0.095 nats悪化し、Eloに対する優位性が失われます。 | [PREDICTION.ja.md](docs/PREDICTION.ja.md) · [en](docs/PREDICTION.en.md) |
...
| **リーグ間の比較** | 予測しやすさは主に戦力差で説明できます。`gain = −0.092 + 1.257 × 戦力差`、欧州プロ22部門でR² **0.735**（リーグ間のばらつきの73.5%を説明）。一方、標準的な均衡指標であるNoll-Scully比は、試合数の少ないリーグの戦力差を過小評価します。 | [LEAGUE_COMPARISON.ja.md](docs/LEAGUE_COMPARISON.ja.md) · [en](docs/LEAGUE_COMPARISON.en.md) |
