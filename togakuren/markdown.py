...
LABELS = {
    ...
    "ja": {
        ...
        "teams_intro": (
            ...
            "選手個人の情報は掲載していません（[{policy}]({policy})）。"
        ),
        "snapshot": "{series} 時点の集計。生成日 {when}。再生成は `{command}`。",
        "final": ("{series}。シーズンは終了しているため、この文書は確定です。"
                  "同じ記録に同じ処理をかければそのまま再現されます。再生成は `{command}`。"),
        "unplayed": "予定されていた全{total}試合のうち{n}試合は不成立で、上の集計には含まれていません。",
        ...
        "fingerprint_note": (
            ...
            "図はダッシュボードと共通で、各クラブの項目にも同じ数値を記載しています。"
        ),
        "axes": "指標",
        "grades": "学年別",
        "history": "過去シーズン",
        "history_note": "連盟のクラブIDで追跡しているため、部の変化がそのまま昇降格を表します。",
        "opponents": "得点した相手",
        ...
        "results_only": (
            "**結果のみのシーズン。** この年、連盟は出場メンバーを記録していません。"
            "そのため出場時間・シュート数と、そこから導かれる指標は算出できません。"
            "以下の順位表が、このシーズンに残っている情報のすべてです。"
        ),
        ...
        "players_intro": (
            ...
            "通常の取り込み処理に通しています。実データとまったく同じコードで作られた出力です。\n\n"
            "実データ版を公開しないのは、ツールの制約ではなく設計上の判断によるものです。"
            "連盟の記録に載っているのはアマチュアの学生であり、選手単位の行は個人情報にあたります。"
            "根拠と実測値は [DATA_POLICY.ja.md](DATA_POLICY.ja.md) にあります。"
        ),
        ...
        "players_note": (
            ...
            "連盟は出場時間そのものを記録していません。"
        ),
        "minutes_heading": "節ごとの出場時間",
        "minutes_note": (
            "1行が選手、1列が節。固定メンバーで戦うチームは同じ選手の行に値が並び、"
            "ターンオーバーの多いチームは多くの行に値が分散します。"
            "ダッシュボードではこれをヒートマップで描いています。"
        ),
        ...
        "trends_title": "シーズン横断",
        "trends_intro": (
            "終了した全シーズンをまとめたものです。2026年より前のシーズンは終了しているため、"
            "以下の表は固定です。同じ記録に同じ処理をかければ、そのまま再現されます。"
            "集計値のみで、個人は登場しません。"
        ),
        "seasons": "シーズン一覧",
        "seasons_note": (
            "選手単位の記録は2022年から。2021年は結果のみのため、シュート数と決定率は"
            "少数の不完全な記録から推定せず、空欄にしています。"
        ),
        "grades_section": "学年別の推移",
        "grades_note": (
            "1つの部の中での、出場時間の割合と90分あたり得点。"
            "数値が低い学年は年によって異なるため、継続的な学年差ではなく世代差とみなせます。"
        ),
        "moves_section": "昇降格",
        "moves_note": (
            "部が変わる前のシーズンと、変わった後のシーズンの1試合あたり勝点。"
            "消化途中のシーズンには印を付け、平均からは除外しています。"
        ),
        "moves_summary": "{label}: {n}件、1試合あたり勝点は平均 {mean:+.2f}。",
        "trajectories": "クラブの推移",
        "trajectories_note": (
            "連盟のクラブIDで追跡した、全クラブのシーズンごとの所属部。"
            "空欄は、そのシーズンにこれらのリーグにいなかったことを示します。"
        ),
